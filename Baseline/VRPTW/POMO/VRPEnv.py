...
from VRProblemDef import get_random_problems_VRPTW, augment_xy_data_by_8_fold
...
class VRPEnv:
    ...
    def load_problems(self, batch_size, aug_factor=1):
        ...
        if not self.FLAG__use_saved_problems:
            depot_xy, node_xy, node_demand, node_earlyTW, node_lateTW, node_servicetime = get_random_problems_VRPTW(batch_size, self.problem_size)
        else:
    ...
    def reset(self):
        ...
        # shape: (batch, pomo)
        # self.length = torch.ones(size=(self.batch_size, self.pomo_size))
        # # shape: (batch, pomo)
